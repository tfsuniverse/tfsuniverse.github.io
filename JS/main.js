...
(function initHeroVideo() {
    ...
    const mobileExtra = document.querySelector('.mobile-extra-content');
    const mobileFill = document.querySelector('.mobile-fill-content');
    const continueSection = document.getElementById('continueWrapper');
    ...
    function showFallback() {
        ...
        if (mobileExtra) mobileExtra.classList.remove('hide');
        if (mobileFill) mobileFill.classList.remove('hide');
        if (continueSection) continueSection.classList.remove('hide');
    }
    ...
    function hideFallback() {
        ...
        if (mobileExtra) mobileExtra.classList.add('hide');
        if (mobileFill) mobileFill.classList.add('hide');
        if (continueSection) continueSection.classList.add('hide');
    }
    ...
    // When video ends → show logo & tagline
    video.addEventListener('ended', () => {
        stopAndReset();
    });

    // Play button → restart with sound
    playBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        playVideo(true);
    });

    // Click logo → restart with sound
    logo.addEventListener('click', (e) => {
        e.stopPropagation();
        playVideo(true);
    });

    // Click anywhere on hero → restart with sound
    ...
    // Scroll detection → stop video & show logo/tagline
    window.addEventListener('scroll', () => {
        const scrollY = window.scrollY || 0;
        if (scrollY > window.innerHeight * 0.25 && isVideoPlaying) {
            stopAndReset();
            hasScrolledAway = true;
        }
    }, { passive: true });

    // Fallback: if video can't play at all
    video.addEventListener('error', () => showFallback());

    // Show UI if video is paused after short delay (e.g., autoplay blocked)
    setTimeout(() => {
